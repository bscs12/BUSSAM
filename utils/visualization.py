...
def visual_segmentation_sets(seg, image_filename, opt):
    ...
    fulldir = opt.output_path + '/' + opt.modelname + '/'
    # fulldir = opt.output_path + '/' + 'GT' + '/'
    if not os.path.isdir(fulldir):
        os.makedirs(fulldir)
    cv2.imwrite(fulldir + image_filename, img)
...
def visual_segmentation_sets_with_pt(seg, image_filename, opt, pt):
    ...
    fulldir = opt.output_path + '/PT10-' + opt.modelname + '/'
    # fulldir = opt.output_path + "/PT3-" + "img" + "/"
    if not os.path.isdir(fulldir):
        os.makedirs(fulldir)
    cv2.imwrite(fulldir + image_filename, img)
...
def visual_segmentation_binary(seg, image_filename, opt):
    ...
    # 创建与原图大小相同的黑色图像
    binary_result = np.zeros_like(img_ori)

    # 将分割部分设置为白色
    binary_result[seg0 > 0] = 255
    ...
    # 保存二值化的分割结果
    cv2.imwrite(fulldir + image_filename, binary_result)
