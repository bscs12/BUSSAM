...
class TwoWayAttentionBlock(nn.Module):
    ...
    def forward(
        ...
        if self.skip_first_layer_pe:
            queries = self.self_attn(q=queries, k=queries, v=queries)  # self.attention among tokens: (b nt c)
        else:
            q = queries + query_pe
        ...
        q = queries + query_pe  # add the original token embedding to enhance the token prompts
        k = keys + key_pe  # enhance the positional information
        attn_out = self.cross_attn_token_to_image(q=q, k=k, v=keys)  # token2image attention: (b nt c)
        ...
        q = queries + query_pe  # (b nt c)
        k = keys + key_pe  # enhance the positional information before attention
        attn_out = self.cross_attn_image_to_token(q=k, k=q, v=queries)  # (b N c)
        keys = keys + attn_out
